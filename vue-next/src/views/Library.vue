...
<template>
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
       <BaseCard v-for="gesture in gestures" :key="gesture.id" class="group hover:border-teal-400/50 transition-all">
          <div class="flex justify-between items-start mb-4">
             <div class="bg-teal-500/10 text-teal-300 px-2 py-1 rounded text-xs font-bold">ACTIVE</div>
          ...
          <div class="flex items-center gap-2 mb-4">
             <div class="flex-1 h-1.5 bg-slate-800 rounded-full overflow-hidden">
                <div class="h-full bg-teal-500 rounded-full" :style="{ width: (gesture.accuracy || 0) + '%' }"></div>
             </div>
             <span class="text-xs text-teal-300">{{ gesture.accuracy || 0 }}%</span>
          </div>
